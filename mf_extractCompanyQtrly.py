...
import pandas as pd
import requests
from seleniumDriver import driver,By
...
def get_company_historical_transcripts(ticker, pagesLoad = 1):
    ...
    result_df = pd.DataFrame([], columns=['Ticker','Company Name','qtr','year','date','link'])
    ...
    for e in enumerate(elements):
        ...
        result_df = pd.concat([result_df,
                            pd.DataFrame({'Ticker':ticker, 'Company Name':name, 'qtr':qtr, 'year':year, 'date':date, 'link':link}, index=[e[0]])])
